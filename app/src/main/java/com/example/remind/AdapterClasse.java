...
public class AdapterClasse extends BaseAdapter {
    ...
    String sharedOfCheckBox ;
    TextView ilghaa ;
    TextView indik ;
    public AdapterClasse(Context context, ArrayList<String> table , String sharedFileName , String sharedOfCheckBox) {
        this.context = context;
        this.table = table;
        this.sharedFileName = sharedFileName ;
        this.sharedOfCheckBox = sharedOfCheckBox ;
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        convertView = LayoutInflater.from(context).inflate(R.layout.list_view_everyday, parent, false);
        // ************************************** Show data **************************************
        TextView description = convertView.findViewById(R.id.writeyourwork);
        description.setText(table.get(position));
        // ************************************** End Show data ***********************************
        // *********************************** Show checkbox *************************************
        SharedPreferences checkShared = context.getSharedPreferences(sharedOfCheckBox , 0);
        SharedPreferences.Editor myeditor = checkShared.edit() ;
        CheckBox checkbox = convertView.findViewById(R.id.checked) ;
        checkbox.setTag(position);
        checkbox.setChecked(checkShared.getBoolean(table.get(position) , false));
        // ******************************* Fin Show checkbox *************************************

        // *************************** Sauvegerder le check dans shared **************************
        checkbox.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                boolean checked = ((CheckBox) v).isChecked();
                if (checked){
                    myeditor.putBoolean(table.get(position), true) ;
                    myeditor.apply() ;
                    checkbox.setChecked(true);
                    Toast.makeText(context.getApplicationContext(),"تم القيام بالعمل اليوم" , Toast.LENGTH_SHORT).show();
                }else { // if he delete the check
                    myeditor.remove(table.get(position)) ;
                    myeditor.apply() ;
                    checkbox.setChecked(false);
                }
            }
        });
        // ************************* Fin Sauvegerder le check dans shared *************************
